...
AStarFinder.prototype.findPath = function(startX, startY, endX, endY, grid) {
    ...
    while (!openList.empty()) {
        ...
        if (node === endNode) {
            ...
            // Cập nhật lại hàm heuristic dựa trên thông tin từ đường đi đã tìm thấy
            for (var j = 0; j < path.length; j++) {
                var pathNode = path[j];
                // Ví dụ: cập nhật lại ước lượng cho các điểm trên đường đi
                pathNode.h = weight * heuristic(abs(pathNode.x - endX), abs(pathNode.y - endY));
                // Nếu cần, bạn có thể thực hiện các điều chỉnh khác tại đây
            }
