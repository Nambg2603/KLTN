...
// Đoạn mã này được đặt trong phạm vi $(document).ready để chắc chắn rằng DOM đã được tải hoàn toàn trước khi thực hiện các thao tác
$(document).ready(function() {


    // Gắn sự kiện click cho nút button
    $('#randomGrayButton').click(function() {
        Controller.clearAll();
        var gridSizeX = Controller.gridSize[0];
        var gridSizeY = Controller.gridSize[1];
        var totalGrids = gridSizeX * gridSizeY;
    
        // Tạo một mảng chứa tất cả các tọa độ có thể trong ma trận
        var allGrids = [];
        ...
        // Thực hiện 50 lần random không trùng nhau
        for (var k = 0; k < 500; k++) {
            // Lấy một số ngẫu nhiên từ 0 đến số lượng tọa độ còn lại
            var randomIndex = Math.floor(Math.random() * allGrids.length);
            // Lấy tọa độ tương ứng từ mảng tọa độ và loại bỏ khỏi mảng
            var randomGrid = allGrids.splice(randomIndex, 1)[0];
            // Lấy tọa độ x và y từ tọa độ ngẫu nhiên
            var randomGridX = randomGrid[0];
            var randomGridY = randomGrid[1];
            // Thay đổi màu sắc của ô tại tọa độ ngẫu nhiên thành màu xám
            Controller.setWalkableAt(randomGridX, randomGridY, false);
        }
    });
